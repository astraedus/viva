...
def _build_system_prompt(session: InterviewSession) -> str:
    ...
    return (
        ...
        f"Conduct the full interview naturally as a conversation. Ask around {cfg.num_questions} questions total. "
        f"You have full control of the interview flow: "
        f"ask a question, listen to the answer, give brief constructive feedback, "
        f"probe deeper if the answer is vague, then transition naturally to the next topic. "
        f"Don't rush — if a candidate gives a short answer, encourage them to elaborate. "
